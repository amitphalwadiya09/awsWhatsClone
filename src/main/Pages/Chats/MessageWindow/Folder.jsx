...
const Folder = ({ onClose }) => {
    ...
    const socket = null; // socket integration is not configured here yet
    const userId = currentUser?.userId || currentUser?._id || currentUser;
    ...
    if (!selectedChat || !selectedChat.participants || !userId) return null;
    ...
    const handleDocChange = async (e) => {
        ...
        try {
            const receiver = selectedChat.participants.find(
                (p) => String(p.userId || p._id) !== String(userId)
            );

            if (!receiver?.userId && !receiver?._id) return;
            ...
            const sent = res?.sentMessage || res;
            dispatch(addMessage({ ...sent, messageStatus: "sent", seenBy: [] }));
            sendSeenMessageSocket(
                selectedChat.conversationId || selectedChat.conversation_id || selectedChat._id,
                userId
            );
            onClose();
            if (socket) socket.emit("send_message", sent);
